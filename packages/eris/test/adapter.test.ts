...
describe("attachMochi", () => {
  it("tracks an application command with its subcommand path", () => {
    ...
    client.emit("interactionCreate", command("config", [{ type: 1, name: "set" }]));
  ...
  it("walks nested subcommand groups", () => {
    const client = fakeClient();
    const { mochi, events } = recorder();
    attach(client, mochi);
    ...
    client.emit(
      "interactionCreate",
      command("config", [
        { type: 2, name: "channel", options: [{ type: 1, name: "set" }] },
      ]),
    );
  ...
  it("falls back to the DM user id and channel type", () => {
    const client = fakeClient();
    const { mochi, events } = recorder();
    attach(client, mochi);
    ...
    expect(events[0]).toMatchObject({ userId: "u9", channelType: "dm", shardId: 0 });
